...
let { selectAllUsers, selectUsers, deleteUsers, createUsers, updateUsers, updatePasswordUsers, findUUID, findEmail, countData } = require("../model/users");
...
let usersController = {
  getAllUsers: async (req, res) => {
    try {
      ...
      commonHelper.response(res, result.rows, 200, "Get Users Data Success", pagination);
    } catch (err) {
      console.log(err);
    }
  },
  ...
  registerUsers: async (req, res) => {
    const { users_name, users_email, users_phone, users_password, users_confirmpassword } = req.body;
  ...
  loginUsers: async (req, res) => {
    ...
    const isValidPassword = bcrypt.compareSync(users_confirmpassword, users.users_confirmpassword);
    if (!isValidPassword) {
