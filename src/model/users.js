...
const createUsers = (data) => {
  const { users_id, users_email, users_password, users_confirmpasswordHash, users_name, users_phone, users_photo } = data;
...
const updateUsers = (data) => {
  ...
  return Pool.query(`UPDATE users SET users_photo = '${users_photo}', users_name = '${users_name}', users_phone = '${users_phone}' WHERE users_id = '${users_id}'`);
};

const updatePasswordUsers = (data) => {
  const { users_id, users_email, users_password, users_confirmpasswordHash } = data;
  return Pool.query(`UPDATE users SET users_password = '${users_password}', users_confirmpassword = '${users_confirmpasswordHash}'WHERE users_id = '${users_id}'`);
};

//FIND EMAIL
const findUUID = (users_id) => {
  return new Promise((resolve, reject) =>
    Pool.query(`SELECT * FROM users WHERE users_id= '${users_id}' `, (error, result) => {
      if (!error) {
        resolve(result);
      } else {
        reject(error);
